...
//
//  Created by Prabhat Tiwari on 25/08/25.
//
...
final class FeedSnapshotTests: XCTestCase {
    ...
    func test_feedWithContent() {
        ...
        assert(snapshot: sut.snapshot(for: .iPhone8(style: .dark, contentSize: .extraExtraLarge)), named: "FEED_WITH_CONTENT_dark_extraExtraLarge")
    }
    ...
    private func feedWithLoadMoreError() -> [CellController] {
        let loadMore = LoadMoreCellController(callback: {})
        loadMore.display(ResourceErrorViewModel(message: "Error message \n multi line"))
        return feedWith(loadMore: loadMore)
    }
    
    private func feedWith(loadMore: LoadMoreCellController) -> [CellController] {
        let stub = feedWithContent().last!
        let cellController = FeedImageCellController(model: stub.viewModel, delegate: stub, selection: {})
